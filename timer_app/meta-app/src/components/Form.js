...
function Form({ onSubmit, submissionId }) {
  const [formData, setFormData] = useState(submissionId ? { ...initialFormState } : initialFormState);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        onSubmit(submissionData); // Update parent component with submitted data
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <input
        type="text"
        placeholder="Machining Time"
        name="machiningTime"
        value={formData.machiningTime}
        onChange={handleInputChange}
      />
      <button type="button" onClick={startSetupTimer}>
        Start Setup Timer
      </button>
      <button
        type="button"
        onClick={startMachiningTimer}
        disabled={!setupStartTime || isMachiningActive}
      >
        Start Machining Timer
      </button>
      <button
        type="button"
        onClick={pauseMachiningTimer}
        disabled={!isMachiningActive}
      >
        Pause/Resume Machining Timer
      </button>
      <button
        type="button"
        onClick={stopMachiningTimer}
        disabled={!isMachiningActive}
      >
        Stop Machining Timer
      </button>
